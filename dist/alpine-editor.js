(function (factory) {
  ...
  var MenuItem = /*#__PURE__*/function () {
    function MenuItem(itemNode, editorView, options, attrs) {
    ...
    _createClass(MenuItem, [{
      ...
      key: "resolveType",
      value: function resolveType(command) {}
    }, {
      key: "update",
  ...
  var MenuItemMark = /*#__PURE__*/function (_MenuItem) {
    _inherits(MenuItemMark, _MenuItem);

    var _super = _createSuper(MenuItemMark);

    function MenuItemMark(itemNode, editorView) {
      var _this;

      var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

      _classCallCheck(this, MenuItemMark);

      _this = _super.call(this, itemNode, editorView, options);
      _this.run = toggleMark(_this.type);
      return _this;
    }

    _createClass(MenuItemMark, [{
      key: "resolveType",
      value: function resolveType(command) {
        return this.schema.marks["".concat(command)];
      }
    }, {
      key: "update",
      value: function update() {
        var containsClass = this.menuItemNode.classList.contains(this.activeClass);
      ...
      value: function isActive() {
        ...
        var _this$state$selection = this.state.selection,
            from = _this$state$selection.from,
            $from = _this$state$selection.$from,
            to = _this$state$selection.to,
            empty = _this$state$selection.empty;
        if (empty) return type.isInSet(this.state.storedMarks || $from.marks()) ? true : false;else return this.state.doc.rangeHasMark(from, to, type);
      }
    }, {
      key: "exec",
      value: function exec() {
  ...
  var MenuItemNode = /*#__PURE__*/function (_MenuItem) {
    _inherits(MenuItemNode, _MenuItem);

    var _super = _createSuper(MenuItemNode);

    function MenuItemNode(itemNode, editorView) {
      var _this;

      var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
      var attrs = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};

      _classCallCheck(this, MenuItemNode);

      _this = _super.call(this, itemNode, editorView, options, attrs);
      _this.run = options.run(_this.type, attrs);
      return _this;
    }

    _createClass(MenuItemNode, [{
      key: "resolveType",
      value: function resolveType(command) {
        return this.schema.nodes["".concat(command)];
      }
    }, {
      key: "isActive",
      value: function isActive() {
        if (this.options.hideable) {
          ...
          var _this$state$selection = this.state.selection,
              $from = _this$state$selection.$from,
              to = _this$state$selection.to,
              node = _this$state$selection.node;
  ...
  var MenuItemAction = /*#__PURE__*/function (_MenuItem) {
    _inherits(MenuItemAction, _MenuItem);

    var _super = _createSuper(MenuItemAction);

    function MenuItemAction(itemNode, editorView) {
      var _this;
  ...
  var ACCEPTED_DATA_ATTRIBUTES = ['level'];
  var OPTIONS_DATA_ATTRIBUTES = ['command', 'activeClass'];

  var MenuBar = /*#__PURE__*/function () {
    function MenuBar(editor, editorView) {
    ...
    _createClass(MenuBar, [{
      key: "initMenuNode",
      value: function initMenuNode(menuNode) {
        var _this = this;

        menuNode.querySelectorAll("button[data-command]").forEach(function (child) {
          var menuItem;

          var attrs = _this.extractNodeAttributes(child);

          var options = _this.extractNodeOptionsFromData(child);

          if (_this.schema.marks["".concat(options.command)]) {
            menuItem = new MenuItemMark(child, _this.editorView, options);
          } else if (_this.commandsManager.hasCommand(options.command)) {
            var command = _this.commandsManager.getCommand(options.command);

            options.run = command.action;
            options.hideable = command.hideable;
            options.activatable = command.activatable;
            menuItem = new MenuItemNode(child, _this.editorView, options, attrs);
          } else if (_this.actionsManager.hasAction(options.command)) {
            options.run = _this.actionsManager.getAction(options.command);
            menuItem = new MenuItemAction(child, _this.editorView, options, attrs);
          } else {
            throw new Error("Could not find Mark, Command or Action for: ".concat(options.command));
          }
      ...
      value: function extractNodeAttributes(node) {
        var attrs = {};
        ACCEPTED_DATA_ATTRIBUTES.forEach(function (attr) {
      ...
      value: function extractNodeOptionsFromData(node) {
        var options = {};
        OPTIONS_DATA_ATTRIBUTES.forEach(function (attr) {
          if (!node.dataset.hasOwnProperty(attr)) {
            return;
          }
  ...
  function initMenuPlugin(editor) {
    return new Plugin({
  ...
  var ActionsManager = /*#__PURE__*/function () {
    ...
    _createClass(ActionsManager, [{
      key: "addAction",
      value: function addAction(key, action) {
  ...
  var CommandsManager = /*#__PURE__*/function () {
    ...
    _createClass(CommandsManager, [{
      key: "addCommand",
      value: function addCommand(key, action) {
        this.commandsRegistry[key] = action;
      }
    }, {
      key: "hasCommand",
      value: function hasCommand(key) {
  ...
  function initNodes(editor) {
    editor.commandsManager.addCommand('heading', {
      action: setBlockType,
      activatable: true
    });
  ...
  var EditorConfig = /*#__PURE__*/function () {
    ...
    _createClass(EditorConfig, [{
      key: "init",
      value: function init() {
        for (var item in this.editor.dataset) {
  ...
  function generateBaseSchema(editor) {
    var emDOM = ["em", 0],
        strongDOM = ["strong", 0],
        codeDOM = ["code", 0];
    var pDOM = ["p", {
      "class": editor.config.pClasses
    }, 0];
    var blockquoteDOM = ["blockquote", {
      "class": editor.config.bqClasses
    }, 0];
    var hrDOM = ["hr", {
      "class": editor.config.hrClasses
    }];
    var preDOM = ["pre", {
      "class": editor.config.preClasses
    }, ["code", 0]];
    var brDOM = ["br"];
    var olDOM = ["ol", {
      "class": editor.config.olClasses
    }, 0];
    var ulDOM = ["ul", {
      "class": editor.config.ulClasses
    }, 0];
    var liDOM = ["li", {
      "class": editor.config.liClasses
    }, 0];
    var nodes = {
      ...
      heading: {
        ...
        toDOM: function toDOM(node) {
          return ["h" + node.attrs.level, {
            "class": editor.config["h".concat(node.attrs.level, "Classes")]
          }, 0];
        }
      },
  ...
  var EditorComponent = /*#__PURE__*/function (_HTMLElement) {
    _inherits(EditorComponent, _HTMLElement);
    ...
    function EditorComponent() {
      ...
      _this.state = null;
      _this.view = null;
      _this.editorNode = null;
      _this.actionsManager = new ActionsManager();
      _this.commandsManager = new CommandsManager();
      _this.debouncedEventHandler = debounce(_this.dispatchCustomInputEvent.bind(_assertThisInitialized(_this)), 250);
      return _this;
    }
    ...
    _createClass(EditorComponent, [{
      ...
      key: "initRootNode",
      value: function initRootNode() {
        var el = this.querySelector('div[data-type="editor"]');
        this.editorNode = el;
        this.editorNode.innerHTML = this.value;
      }
    }, {
      key: "initMenuNode",
      value: function initMenuNode() {
      ...
      value: function generateEditorState() {
      ...
      value: function createEditorView() {
        var attributes = {};
      ...
      value: function dispatchCustomInputEvent() {
      ...
      value: function getPluginsList() {
        return [keymap(baseKeymap), initMenuPlugin(this)];
      }
    }]);
